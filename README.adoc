...
Personal Development Environment crafted from scratch with every Human-Computer
Interaction engineered to fit personal needs and offer supreme comfort to
improve workflow by reducing the distance between intent and execution.
...
1. https://github.com/neovim/neovim[*Neovim*]: Base Neovim
2. https://git-scm.com/[*git*]: For git integration and plugin manager | `lazy.nvim`

Installing the following dependencies activates plugins and features that require them:

[start=3]
3. https://gcc.gnu.org/[*GCC*]: For compiling treesitter language parsers | `nvim-treesitter`
4. https://github.com/tree-sitter/tree-sitter-cli[*tree-sitter-cli*]: Incremental language parsers | `nvim-treesitter`
5. https://github.com/nodejs/node[*nodejs*]: For Language Servers and Formatters | `mason-tool-installer.nvim`
6. https://github.com/tapio/live-server[*live-server*]: For local HTML Server | `Execute_order_69`
7. https://github.com/BurntSushi/ripgrep[*ripgrep*]: For live grep | `snacks.nvim`
8. https://github.com/sharkdp/fd[*fd*]: For Find Files | `snacks.nvim`
9. https://github.com/vifm/vifm[*vifm*]: File Manager | `vifm.vim`
10. https://github.com/jesseduffield/lazygit[*lazygit*]: Git UI | `snacks.nvim`
11. https://github.com/kovidgoyal/kitty[*kitty*]: Kitty's Graphics Protocol for inline image/diagram rendering | `image.nvim`
12. https://github.com/mermaid-js/mermaid-cli[*mermaid-cli*]: For rendering inline Mermaid Diagrams | `diagram.nvim`
13. https://github.com/InioX/matugen[*matugen*]: For dynamic color support throughout the UI | `init.lua`
14. ...
15. *Burgher King Foot Lettace*: the last thing you want in your Burgher King burgher is someone’s fut funguès. But as it turns out, that might be what you GÆT.
...
If any config is already present, it backups your config and put Hunter-Nvim in place.
...
git clone https://github.com/MidHunterX/Hunter-Nvim $ME && nvim
----
...
* [.line-through]#Suggested Compiler for Tree-sitter: *Zig Compiler*#

WARNING: After `nvim-treesitter` rewrite, it uses `tree-sitter-cli` for installing parsers which don't support Zig Compiler. Windows now need *MSYS2* or *MinGW*.
...
[source,pwsh]
----
git clone https://github.com/MidHunterX/Hunter-Nvim $env:localappdata\nvim; nvim
----
====
...
Uninstalls Hunter-Nvim and returns back to previous config if any.
...
Useful when having Gradle issues upon flutter build.
...
First install Android SDK. If you want to build native Android apps, you'll need Android NDK also (around 2.5 GB).
...
Optionally adding users to the 'flutter' group would give them direct access to '/opt/flutter'

[source,bash]
----
sudo usermod -a -G flutter
----

Flutter usually installs Android SDK in it's own managed directory. If you installed SDK by yourself manually, it's usually in `/opt/android-sdk` which is not always accessible. To give flutter permission:
To avoid problems down the line, you can own it.
...
Accept all licenses with "The Power of YES" - John Seed (Far Cry 5)
...
Now check everything is good with `flutter doctor -v`. You should be good to go. But if the licenses are still not accepted after the above command, it's just a permission problem.

SDK is located at `/opt/android-sdk` which can only be accessed with root privileges. So, you need sudo. But, flutter don't accept running with sudo privileges. flutter doctor runs `sdkmanager` (android-sdk-cmdline-tools-latest) under the hood so, just use it directly with sudo instead:

[source,bash]
----
yes | sudo sdkmanager --sdk_root=/opt/android-sdk --licenses
----
...
Install https://github.com/cushycush/qml-language-server[QML Language Server] with Quickshell with:
...
After installation, qml-language-server will be activated upon opening a `.qml` file. This is much more complete than official qmlls.

If you prefer to use the official qmlls for Qt without Quickshell, just uninstall qml-language-server and install `qmlls` via `:Mason`.
====
...
[discrete]
=== LSP Calls
Use the following commands to view incoming and outgoing call list
...
.What all does this function call?
[source,lua]
----
...
If the cursor is on an empty line, the same `H` and `L` acts as paragraph
navigation jumping through gaps in vertical direction.
====
...
* Auto-installs necessary LSP servers, treesitter parsers etc. on first run

=== 🎉 Commit Messages

`gitcommit.lua`

https://gitmoji.dev/[image:https://img.shields.io/badge/gitmoji-%20😜%20😍-FFDD67.svg?style=flat-square[Gitmoji]]
https://conventionalcommits.org/[image:https://img.shields.io/badge/Conventional%20Commits-1.0.0-%23FE5196?style=flat-square&logo=conventionalcommits&logoColor=white[Conventional Commits]]

Editing `COMMIT_EDITMSG` activates a bespoke `cmp` completion for commit messages.
The commit message cmp combines the structure of https://www.conventionalcommits.org/[Conventional Commits] with the aesthetics of https://gitmoji.dev/[Gitmoji] 1:1.
...
.`ℹ️ Other Treesitter Related Features`
[%collapsible]
====
* `lang_flutter-tools.lua`: *Flutter LSP* integration for Dart. Activates only when a Dart file is open.
* `treesitter_context.lua`: Show context code on top of buffer. Breadcrumbs are almost useless in comparison.
* Use `:InspectTree` to view the full syntax tree.
====
...
*Code indentation lines* which highlights the current https://en.wikipedia.org/wiki/Scope_(computer_science)[scope]. The scope is not the current indentation level! Instead, it is the indentation level where variables or functions are accessible. This depends on the language you are writing. The highlight colors are matched with the colors of brackets for a more visually cohesive experience. `#Hidden_UI: Indent line as variable scope`

`neural-filepicker.lua`

`<space> <space>` to open a file picker that predicts the next file you are going to open. A neural network trains on your file selections to rank results by what you're most likely to open next, combining fuzzy matching with contextual signals like buffer state, directory proximity, frecency, and file-to-file transition patterns.
...
Custom regex autocompletion powered by `regex.lua` is enabled upon the execution of `<leader>ur`.
...
* `flash.lua`: *Leap* to any text with `s` (Search) and select any Treesitter node with `S` (Select TS). These are also available as an adverb in vim motion grammar using `r` (Remotely) and `R` (Remotely TS) instead (e.g. yr<search_text>iw).
...
* `treesitter_autotag.lua`: Autoclose HTML tags, rename HTML tags with normal insert mode editing.
====
...
* `scrollbar.lua`: Aesthetic scrollbar using Matugen colorscheme. Get a birds eye view of all important information in the buffer like changes, diagnostics, git status etc.
====
...
| `<Space>`    | Leader
| `<leader>w`  | Write File
| `<leader>er` | Erase Regex Highlight
| `<leader>eM` | Erase Marks
| `<leader>rr` | Highlight & Replace Word
| `<leader>yy` | Yank Entire Buffer
| `<leader>yf` | Yank File Relative Path (Normal & Visual)
| `<leader>yF` | Yank File Absolute Path (Normal & Visual)
| `<leader>yc` | Yank Context for LLMs (Normal & Visual)
| `<leader>yC` | Yank Context for LLMs (Normal & Visual)
