...
[source,bash]
----
D=~/.config/nvim;if [ -d $D ]; then mv $D ${D}_backup/;fi
git clone https://github.com/MidHunterX/HunterX-PDE $D --depth 1 && nvim
----

.fish
[source,fish]
----
set D ~/.config/nvim; test -d $D; and mv $D {$D}_backup
git clone https://github.com/MidHunterX/HunterX-PDE $D --depth 1 && nvim
----
...
.`Alternative Behaviour Implementations`
[%collapsible]
====
...
* ✅ Integrates very well into how terminal autocompletion works
* ❌ Can create a conflict while using tab jumps on snippets
...
* ✅ Does not conflict with other keys since arrows are used less
* ❌ Uncomfortable if you are touch typing

[discrete]
=== Autocompletion: Emacs Style
...
* ✅ Comfortable on a QWERTY layout
* ❌ Cannot autocomplete without expanding with space

[discrete]
=== Autocompletion: Vim+Emacs
...
* ✅ Integrates well into how Vim works + TMUX
* ✅ Can autocomplete without expanding with space
* ✅ Comfortable on a QWERTY keyboard layout
