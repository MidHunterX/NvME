...
Language Server Protocol is developed by Microsoft, alongside collaboration
with Red Hat and Codenvy. LSP defines the protocol used between an editor and a
Language Server that provides language features like autocomplete, goto
definition etc.
...
== 📑 Tab out of Brackets/Quotes (neotab.nvim)

Migrated from `tabout.nvim` to `neotab.nvim`. Unlike `tabout.nvim` that tabout
out of treesitter nodes, `neotab.nvim` uses simple logic to move in and out of
pairs or find the next best matching pair.
...
state of the parsed treesitter tree. It offers a different, less fancy but more
predictable and consistent method for tabbing out of pairs.
