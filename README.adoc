...
There's a full kill switch system which disables features safely based on the available requirements in PATH.
~And it's super hard to set up all the requirements on Windows.~
Use winget to install everything.
...
NOTE: Setting env `NVIM_OPEN_SIDEBAR=1` will auto open the sidebar only after opening a file; not at the dashboard.
