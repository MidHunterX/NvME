...
Note: `i` and `C-i` are used to accept completion which the user have selected.
`CR` accepts the first entry by default or the one preselected by LSP/User.

If accepted entry is a snippet, use `TAB` & `S-Tab` for jumping around.
